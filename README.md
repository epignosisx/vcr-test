...
Requires **Node.js 22 or newer** (`@mswjs/interceptors`, which powers the interception, requires
it). For Node 18 or 20, use `vcr-test` 1.x.
...
Using `await using` (requires TypeScript 5.2+):
...
`await using` additionally needs TypeScript 5.2+. On older TypeScript, use the callback-based API:
...
| `all`    | Re-record from scratch: ignore any existing cassette, make every call live, and replace the file with this run's traffic. Equivalent to deleting the cassette and recording it again. |
...
You can edit cassettes by hand — they're just YAML. `content-length` is restated from the body on
replay, so you do not need to keep it in step when you change one; vcr-test warns when it has to
correct a cassette, and re-recording clears the warning.
...
Responses can carry sensitive data too — an upstream that echoes a token back, or PII in a body
that is about to be committed to a cassette. Assign a response masker to rewrite it at record
time. It receives the response, and the request that response answered:
...
vcr.responseMasker = (res, req) => {
  // `req` scopes the mask to one endpoint, so unrelated bodies are left alone.
  if (!req.url.includes('/accounts/')) return;
...
The second parameter is optional — drop it to mask every response:

```ts
vcr.responseMasker = (res) => {
  res.body = res.body.replace(/\d{16}/g, 'MASKED_CARD_NUMBER');
};
```

Anything on the request is fair game for the decision — url, method, headers, body:

```ts
vcr.responseMasker = (res, req) => {
  if (req.method === 'POST' && req.headers['content-type']?.startsWith('application/json')) {
    res.body = maskJson(res.body);
  }
};
```

Two things to know about that request:

- **It is read-only.** It is context for the decision, not a second place to mask requests — use
  `requestMasker` for that. TypeScript will reject assigning to it.
- **It has already been through `requestMasker`**, so it is the request as the cassette will carry
  it. If your request masker rewrites the url, branch on what survives it — a check against a real
  account id or token will never match.

Rewrite the body freely: `content-length` is restated from whatever the body ends up being, so a
masker never has to keep framing headers in step with its own edits. A masker that throws will
fail the recording, so guard against bodies that are not the shape you expect (a non-JSON error
page, an empty body).
...
1. Change the cassette manually, after all it is just YAML. `content-length` is restated from the body on replay, so you do not need to keep it in step.
...
3. Change VCR's `mode` to `all` (or set `VCR_MODE=all`) to re-record the whole cassette, without having to delete the file first.
